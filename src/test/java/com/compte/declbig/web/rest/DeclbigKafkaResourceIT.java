package com.compte.declbig.web.rest;

import com.compte.declbig.config.KafkaProperties;
...
class DeclbigKafkaResourceIT {
    ...
    void setup() {
        ...
        DeclbigKafkaResource kafkaResource = new DeclbigKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/declbig-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/declbig-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
